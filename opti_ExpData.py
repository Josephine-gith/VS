...
# Optimisation du paramètre a


def objective(a, df_exp):
    a = a[0]
    if a > 200 or a <0.1 :
        return 1e6
    ...
    # Calcul du modèle avec a variable
    df["1 - h_inf/h0 KV"] = df.apply(
        lambda r: modeleKV_solve(
            r["h0 [m]"], r["r0 [m]"], r["r [kg/m³]"], r["k [Pa s]"], r["Bn"], a=a, G=0
        )[0],
        axis=1,
    )
...
res = minimize(
    ...
    x0=a0,
    args=(df_exp,),
    bounds=[(0.1, 500)],  # bornes physiques raisonnables
    method="L-BFGS-B",
)

a_opt = res.x[0]

print(f"a optimal = {a_opt:.3f}")
print(f"Erreur minimale = {res.fun:.3e}")


# Affichage avec le a optimisé

df["1 - h_inf/h0 KV"] = df.apply(
    lambda r: modeleKV_solve(
        r["h0 [m]"], r["r0 [m]"], r["r [kg/m³]"], r["k [Pa s]"], r["Bn"], a=a_opt, G=0
    )[0],
    axis=1,
)
